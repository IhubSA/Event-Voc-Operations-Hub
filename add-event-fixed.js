...
import { getOrgId } from './org-branding.js';
...
export class AddEventModal {
  ...
  generateEventCode(eventType, date) {
    // Format: First letter of type + month + day + a short random suffix
    // (e.g., S0825-K3F). events.code is unique across the WHOLE platform
    // (every club), so the suffix is needed to avoid collisions between
    // different clubs creating a similar event on the same day.
    const typePrefix = eventType ? eventType.substring(0, 1).toUpperCase() : 'E';
    const dateObj = new Date(date);
    const month = String(dateObj.getMonth() + 1).padStart(2, '0');
    const day = String(dateObj.getDate()).padStart(2, '0');
    const suffix = Math.random().toString(36).slice(2, 5).toUpperCase();
    return `${typePrefix}${month}${day}-${suffix}`;
  }
  ...
  setupEventListeners(onEventCreated, onCancel) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // Insert event into Supabase. events.code is unique platform-wide,
        // so on the astronomically rare chance of a collision, regenerate
        // the suffix and retry once before giving up.
        ...
        let { data, error } = await supabase.from('events').insert(buildPayload(eventCode)).select();

        if (error && error.code === '23505') {
          const retryCode = this.generateEventCode(eventType, eventDate);
          ({ data, error } = await supabase.from('events').insert(buildPayload(retryCode)).select());
        }
